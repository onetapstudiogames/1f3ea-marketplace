...
// two such fetches have run in the same process can race that teardown on
// some Windows Node builds, crashing with an `UV_HANDLE_CLOSING` assertion
// (src/win/async.c) and a garbage exit code (0xC0000409) instead of the
// clean, correct exit this script always means to produce. The drain
// narrowed the race (measured failure threshold: 20-40ms idle) but did not
// close it -- under real CPU contention the shipped 100ms constant still
// crashed a measured fraction of runs, because there is no signal available
// to wait ON: `process.getActiveResourcesInfo()` does not even report the
// offending handle at that point. The only strategy that never raced in
// that same testing, idle or loaded, is not calling process.exit() at all:
// every refusal and every success below sets `process.exitCode` and then
// either returns from `main()` or throws `SetupRefusal`, which `main()`'s
// caller catches and swallows, so the module falls off the end and Node
// exits naturally with whatever `process.exitCode` was last set to -- the
// same thing this script's own final success path already did.
//
// `SetupRefusal` exists only to unwind a refusal detected deep inside a
// helper (`verifyStoredKeyOrRefuse`, itself called from `report`, itself
// called from `finishAsRepair`) back out to `main()` without every
// intermediate function needing to manually check and re-propagate a
// "stop now" signal -- `process.exitCode` is always set at the throw site,
// never at the catch site, so the exit code this produces is exactly the
// one the refusal chose.
...
async function main() {
  ...
  // second time in the same run -- one fewer real network round trip. This
  // used to also matter for avoiding a rare Node/libuv shutdown crash on
  // some Windows builds (`UV_HANDLE_CLOSING` assertion in src/win/async.c,
  // observed when two AbortSignal.timeout()-gated fetches ran in the same
  // process); that crash class is now structurally impossible regardless of
  // fetch count, since this file never calls `process.exit()` (see the
  // comment above `class SetupRefusal`), so the fewer-round-trips benefit
